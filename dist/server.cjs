...
__export(server_exports, {
  ...
  POST: () => POST,
  PUT: () => PUT,
  agenteParaRuta: () => agenteParaRuta,
  almacenAgentesDeArchivo: () => almacenAgentesDeArchivo,
  almacenDeArchivo: () => almacenDeArchivo,
  createAgentRoutes: () => createAgentRoutes,
  createAgentePublicoRoute: () => createAgentePublicoRoute,
  createAgentesAdminRoutes: () => createAgentesAdminRoutes,
  createAgentesListado: () => createAgentesListado,
  endpointDe: () => endpointDe,
  saneaExclusiones: () => saneaExclusiones,
  saneaRuta: () => saneaRuta,
  saneaSlug: () => saneaSlug
});
...
function cifrar(texto, clave) {
  ...
  const cipher = import_crypto.default.createCipheriv("aes-256-gcm", clave, iv);
...
function descifrar(payload, clave) {
  ...
  const decipher = import_crypto.default.createDecipheriv("aes-256-gcm", clave, Buffer.from(ivB64, "base64"));
...
function almacenDeArchivo(ruta = ".bivoo-agent.json") {
  const rutaAbsoluta = import_path.default.isAbsolute(ruta) ? ruta : import_path.default.join(process.cwd(), ruta);
  const clave = () => claveMaestra();
  return {
    async leer() {
      try {
        const texto = await import_fs.promises.readFile(rutaAbsoluta, "utf8");
        const k = clave();
    ...
    async guardar(config) {
      ...
      await import_fs.promises.writeFile(rutaAbsoluta, k ? cifrar(texto, k) : texto, "utf8");
    }
  };
}
...
function almacenAgentesDeArchivo(ruta = ".bivoo-agentes.json") {
  const rutaAbsoluta = import_path2.default.isAbsolute(ruta) ? ruta : import_path2.default.join(process.cwd(), ruta);
  async function leerTodos() {
    try {
      const texto = await import_fs2.promises.readFile(rutaAbsoluta, "utf8");
  ...
  async function guardarTodos(agentes) {
    await import_fs2.promises.writeFile(rutaAbsoluta, JSON.stringify(agentes, null, 2), "utf8");
  }
  return {
    listar: leerTodos,
    async crear(datos) {
